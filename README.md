...
### แดชบอร์ดเจ้าหน้าที่ (หน้าเว็บ `/`)

เปิดหน้าแรกของแอป (เช่น `https://<your-domain>/`) จะพบแดชบอร์ดสำหรับเจ้าหน้าที่สหกรณ์:

- **ภาพรวมธุรกรรม** — ยอดรวม/ยอดเดือนนี้, กราฟแยกหมวดหมู่ (ซื้อหุ้น, ชำระหนี้, ฝากเงิน ฯลฯ), แนวโน้มรายเดือน
- **ตารางรายการ** — กรองตามช่วงวันที่/หมวด/สถานะยืนยันตัวตน, แบ่งหน้า, แก้ไข/ลบ, ส่งออก CSV (มี BOM เปิดใน Excel ได้ทันที)
- **คิวตรวจสอบตัวตน** — ปุ่มลัด "⚠️ คิวตรวจสอบ" กรองเฉพาะรายการที่ `memberVerified = false` (เลขสมาชิกไม่ตรงทะเบียนตอนบันทึก) กด "ยืนยันตัวตน" แล้วระบบจะ (1) ยืนยันทุกรายการของเลขสมาชิกนั้น (2) เพิ่มสมาชิกเข้า `MemberRoster` หรือผูก LINE id ให้แถวเดิม เพื่อให้รายการถัดไปของสมาชิกคนนี้ผ่านการตรวจอัตโนมัติ
- **ทะเบียนคำขอบริการ** — ประวัติการส่งต่อเอกสาร/คำขอถึงเจ้าหน้าที่ทุกครั้ง (ตาราง `ServiceRequestLog` — บันทึกทั้งกรณีส่งสำเร็จ, ส่งไม่สำเร็จ, และยังไม่ตั้งค่าผู้รับ) พร้อมลิงก์รูปเอกสารและเบอร์ติดต่อกลับ
- **บันทึกรายการโดยเจ้าหน้าที่** — ฟอร์มเพิ่มรายการเอง (เช่น ชำระที่เคาน์เตอร์) กรอกเลขสมาชิกแล้วระบบตรวจกับทะเบียนให้แบบเดียวกับบอท
- **ฐานความรู้ของบอท** — แก้อัตราดอกเบี้ย/สวัสดิการ/ข้อมูลติดต่อที่บอทใช้ตอบสมาชิกได้จากแดชบอร์ดโดยตรง (ตาราง `KnowledgeEntry` — seed ค่าปัจจุบันให้ตอน migrate) มีผลภายในประมาณ 1 นาที ไม่ต้องแก้โค้ด/deploy ใหม่อีกต่อไป ถ้าตารางว่างหรืออ่านไม่ได้ บอทจะ fallback ไปใช้ค่า default ที่ฝังไว้ใน `lib/knowledge.ts` เสมอ

**การป้องกัน**: ทุกหน้า/ทุก API (ยกเว้น `/api/line/webhook` ซึ่งตรวจ signature ของ LINE อยู่แล้ว) ถูกป้องกันด้วย Basic Auth ผ่าน `middleware.ts` — ต้องตั้ง `DASHBOARD_USER` / `DASHBOARD_PASSWORD` ก่อน มิฉะนั้นแดชบอร์ดจะปิดตัวเอง (ตอบ 503) เพื่อไม่ให้ข้อมูลการเงินสมาชิกเปิดสาธารณะโดยไม่ตั้งใจ
...
| `LINE_FORWARD_LOAN_ID` | (ไม่บังคับ) LINE user ID ของผู้รับผิดชอบเรื่องกู้เงินโดยรวม — ใช้เป็น fallback เมื่อยังไม่มี contact เฉพาะหน่วยงานนั้นใน `LoanDistrictContact` |
...
### แยกเรื่องกู้เงินตามหน่วยงาน (ไม่บังคับ)

การกู้เงินจะพยายามหาผู้รับผิดชอบเฉพาะหน่วยงานของสมาชิกก่อน (จากตาราง `LoanDistrictContact` ซึ่ง match แบบตรงตัวกับ `MemberRoster.unitName` — ไม่ได้ตัดเฉพาะรหัสอำเภอ เพราะการมอบหมายงานจริงของสหกรณ์เป็นระดับหน่วยงาน ไม่ใช่ระดับอำเภอ และเจ้าหน้าที่หนึ่งคนมักดูแลหลายหน่วยงาน) แล้วค่อย fallback ไป `LINE_FORWARD_LOAN_ID` ถ้ายังไม่มี ตารางนี้เริ่มต้นว่างเปล่าโดยตั้งใจ

ถ้ามีสเปรดชีตรายชื่อผู้รับผิดชอบอยู่แล้ว (ชีตชื่อ `รับผิดชอบ` คอลัมน์: 1 UserID, 2 สังกัด, 3 ตัวย่อ, 4 ผู้รับผิดชอบ) รันสคริปต์นี้เพื่อนำเข้าทั้งไฟล์ในครั้งเดียว — จะรายงานด้วยว่าชื่อหน่วยงานไหนไม่ match กับ `MemberRoster.unitName` แบบตรงตัว (หน่วยงานนั้นจะยัง fallback ไป `LINE_FORWARD_LOAN_ID` จนกว่าชื่อจะตรงกัน):

```bash
npx tsx scripts/import-loan-contacts.ts <path-to-xlsx>
```

หรือเพิ่ม/แก้ทีละหน่วยงานด้วย Prisma Studio (`npx prisma studio`) หรือ SQL ตรงๆ:

```sql
INSERT INTO "LoanDistrictContact" (id, "unitName", "lineUserId", "createdAt", "updatedAt")
VALUES (gen_random_uuid()::text, 'บำนาญ อ.เมือง นค.', 'Uxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', now(), now());
```
...
เมื่อสมาชิกถามอัตราดอกเบี้ยเงินฝาก/เงินกู้, สวัสดิการ, ข้อมูลติดต่อ, หรือขอแบบฟอร์ม บอทจะตอบจาก**ฐานความรู้ในตาราง `KnowledgeEntry`** (ประกอบเข้า system prompt ตอน runtime ผ่าน `lib/knowledge.ts` มี cache ในหน่วยความจำ 60 วินาที) ไม่ใช่การค้นเว็บสด — **แก้ไขได้จากแดชบอร์ดเจ้าหน้าที่โดยตรง ไม่ต้อง commit/deploy** ถ้าตารางว่างหรืออ่านไม่ได้ บอทจะใช้ค่า default ที่ฝังไว้ใน `lib/knowledge.ts` (ชุดเดียวกับที่ migration seed ให้) เพื่อให้ตอบได้เสมอ
...
```bash
npx tsc --noEmit
npm test          # unit tests (vitest) — ตัวกรองลิงก์, ตัวกรองรายการ
npm run build
```
