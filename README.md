...
| `LINE_FORWARD_TARGET_ID` | (ไม่บังคับ) LINE group/user ID ที่จะส่งต่อคำขอของสมาชิก เมื่อมีคนส่งเอกสารประกอบ (สลิปเงินเดือน, สำเนาบัตร, ทะเบียนบ้าน, ทะเบียนสมรส) แทนสลิปโอนเงิน — ถ้าไม่ตั้งค่า บอทจะขอโทษและแนะนำให้ติดต่อสำนักงานสหกรณ์โดยตรงแทน ไม่ส่งต่อจริง |

วิธีหา `LINE_FORWARD_TARGET_ID`: เพิ่มบอทเข้ากลุ่ม LINE ปลายทาง (เช่น กลุ่มฝ่ายสินเชื่อ) จากนั้นเพิ่ม `console.log(event.source)` ชั่วคราวไว้ต้นๆ ของ `POST` ใน `app/api/line/webhook/route.ts` (ก่อนเงื่อนไขที่กรอง `event.source?.type !== "user"` ทิ้ง เพราะปกติ event จากกลุ่มจะถูกกรองออกไปก่อนโดยไม่มีการบันทึกอะไรเลย) แล้วให้ใครสักคนพิมพ์อะไรก็ได้ในกลุ่มนั้น ดู `groupId` จาก log ที่ขึ้น คัดลอกมาใส่ตัวแปรนี้ แล้วอย่าลืมลบ `console.log` ที่เพิ่มไว้ออกก่อน deploy จริง
