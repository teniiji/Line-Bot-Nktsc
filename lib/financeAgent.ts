...
function buildSystemPrompt(
  ...
  if (pending) {
    ...
    if (next === "member_info") {
      flowNote = `\n\nหมายเหตุระบบ (สำคัญ): มีธุรกรรมค้างอยู่ (${pending.category ?? "ยังไม่ทราบหมวดหมู่"}, ${amountNote}) กำลังรอข้อมูลสมาชิก (ชื่อ-นามสกุล และเลขสมาชิก) — นี่คือครั้งแรกที่ผู้ใช้คนนี้ทำธุรกรรม ถ้าข้อความปัจจุบันของผู้ใช้มีชื่อ-นามสกุลและเลขสมาชิกอยู่แล้ว ให้เรียก submit_member_info ทันทีด้วยข้อมูลนั้น ถ้ายังไม่มีในข้อความนี้ ให้ถามชื่อ-นามสกุลและเลขสมาชิกอีกครั้งสั้นๆ ห้ามเรียก report_transaction ซ้ำในข้อความนี้`;
    } else if (next === "slip") {
...
async function reportTransaction(
  ...
  const next = computeNextRequirement(lineUser, pending);
  if (next === null) {
    return await finalizeTransaction(ctx.lineUserId, pending, lineUser as LineUserInfo);
  }
